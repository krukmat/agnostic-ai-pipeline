...
def _run_dspy(concept: str) -> dict[str, str]:
    ...
    if db.enabled:
        db.log_event("ba_start", role="ba", message=f"Generating requirements for: {concept}")
        db.save_artifact("ba", "requirements", data)
        db.log_event("ba_end", role="ba", message="BA requirements generated successfully")
...
async def generate_requirements(concept: str) -> dict[str, str]:
    ...
    if db.enabled:
        ...
        if req_path.exists():
            ...
            db.log_event("ba_start", role="ba", message=f"Generating requirements for: {concept}")
            db.save_artifact("ba", "requirements", payload)
            db.log_event("ba_end", role="ba", message="Requirements generated")
