...
- Al cerrar un branch de feature/proyecto y hacer merge a `main`, compilar la documentación interna relevante en un único documento didáctico de “memoria de proyecto” (por ejemplo, bajo `docs/`) y referenciarlo desde el `README.md`. El nombre de ese documento debe estar claramente relacionado con el feature (por ejemplo, `docs/orchestrator_cot_project_memory.md`, `docs/db_layer_project_memory.md`) para ubicar rápido el contexto. Una vez validada esa memoria, revisar y eliminar los demás documentos internos que no formen parte de ella, previa confirmación explícita del usuario.
