...
- **Roles** – Business Analyst, Product Owner (alignment stage), Architect, Developer, QA, and Orchestrator collaborate sequentially.
...
flowchart LR
    ...
    BA --> PO[Product Owner (alignment stage)]
    PO --> ARCH[Architect]
    ARCH --> DEV[Developer]
    DEV --> QA[QA]
...
### Product Owner Alignment Stage

- **Goal** – Validate that requirements generated by the Business Analyst adhere to the product vision before technical planning begins.
- **Outputs (planned)** – Vision summary (`planning/product_vision.yaml`) and alignment review (`planning/product_owner_review.yaml`).
- **Status** – The automated loop currently executes BA → Architect → Dev → QA; the Product Owner review is being formalised to sit between BA and Architect for upcoming iterations.
...
- **CONCEPT** – Plain-text business goal consumed by the Business Analyst (and slated for Product Owner review).
...
Each call orchestrates BA → (planned) Product Owner alignment → Architect → Dev → QA, snapshots the full state, and writes a `summary.json` with story status counts and configuration flags.
...
2. **Product Owner alignment (planned)** – Product Owner validates requirements against the vision (`planning/product_vision.yaml`, `planning/product_owner_review.yaml`).
3. **Architecture & stories** – Architect creates PRD, architecture, epics, stories, and tasks.
4. **Development** – Dev agent iterates through stories, updating `project/` code and tests.
5. **Quality gate** – QA executes pytest/Jest suites and enforces TDD:
...
6. **Snapshot** – `artifacts/iterations/<iteration>/` stores planning, project, and `summary.json`.
7. **Follow-up** – Use `make loop MAX_LOOPS=1` to retry blocked stories with the refined criteria.
